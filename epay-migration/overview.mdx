...
| `/epay/api.php?act=refund` | `GET` or `POST` | Refund compatibility path | Submits refunds for eligible Epay-source orders when refund access is enabled. |
...
4. Map your payment `type` values to supported values such as `alipay`, `wxpay`, `creditcard`, `crypto`, or `paynow`.
...
7. Use [`api.php?act=order`](/epay-migration/api-php) for compatibility order queries, and [`api.php?act=refund`](/epay-migration/api-php) only for Epay-source order refunds.
